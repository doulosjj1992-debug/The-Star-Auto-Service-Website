...
const BRAND_BLUE = "#0b5ed7", BRAND_BLUE_DARK = "#094db4";
const SERVICE_GREEN = "#28a745", SERVICE_GREEN_DARK = "#218838";
const CONTAINER_MAX = "1200px";
...
function Loop({ title, items, fast, className }: LoopProps) {
  ...
  useEffect(() => {
    ...
    const onVis = () => {
      ...
      if (sib) sib.style.animationPlayState = s;
    };
  ...
  return (
    <section className={`auto_loop_outer ${className || ""}`} aria-label={title} aria-live="off">
        ...
        <div className="auto_loop_mask">
          <div className={speedClass} ref={trackRef}>
            <ul className="auto_loop_group">
              {items.map((it, i) =>
                it.href ? (
                  <li key={i}><a className="auto_chip" href={it.href} target="_blank" rel={it.rel || "noopener noreferrer"}>{it.text}</a></li>
                ) : (
      ...
      <style jsx>{`
        ...
        .auto_bar_title{text-align:center;font-size:13px;text-transform:uppercase;letter-spacing:1.5px;color:#5a6772;margin-bottom:12px;font-weight:600}
        ...
        .auto_chip{display:inline-block;padding:9px 18px;margin:0 7px;background:linear-gradient(135deg, ${BRAND_BLUE} 0%, ${BRAND_BLUE_DARK} 100%);color:#fff;text-decoration:none;border-radius:999px;font-size:14px;font-weight:500;transition:transform .2s,box-shadow .2s,filter .2s;box-shadow:0 2px 8px rgba(11,94,215,.18);outline:none}
        ...
        :global(.makes) .auto_chip{background:linear-gradient(135deg, ${SERVICE_GREEN} 0%, ${SERVICE_GREEN_DARK} 100%);box-shadow:0 2px 8px rgba(40,167,69,.18)}
        :global(.makes) .auto_chip:hover,:global(.makes) .auto_chip:focus-visible{box-shadow:0 6px 18px rgba(40,167,69,.25)}
      `}</style>
    </section>
  );
}
